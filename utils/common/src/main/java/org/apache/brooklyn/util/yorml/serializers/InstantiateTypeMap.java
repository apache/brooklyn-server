...
public class InstantiateTypeMap extends YormlSerializerComposition {

    private static final String MAP = YormlUtils.TYPE_MAP;
    ...
    public class Worker extends InstantiateTypeWorkerAbstract {
            ...
            if (value instanceof Iterable) {
                ...
                for (Object o: (Iterable<?>) value) {
                    ...
                    if (o instanceof Map) {
                        ...
                        if (ek2==null) {
                            ek2 = converter.read(new YormlContextForRead(ek1, newPath+"/key", genericKeySubType));
                        }
                        ev2 = converter.read(new YormlContextForRead(ev1, newPath+"/value", genericValueSubType));
                        jom.put(ek2, ev2);
                    } else {
                ...
                for (Map.Entry<?,?> me: ((Map<?,?>)value).entrySet()) {
                    Object v = converter.read(new YormlContextForRead(me.getValue(), context.getJsonPath()+"/"+me.getKey(), genericValueSubType));
                    jom.put(me.getKey(), v);
                }
        ...
        protected boolean parseExpectedTypeAndDetermineIfNoBadProblems(String type) {
            if (isJsonMarkerType(type)) {
                genericKeySubType = YormlUtils.TYPE_JSON; 
                genericValueSubType = YormlUtils.TYPE_JSON; 
            } else {
        ...
        public void write() {
            ...
            if (allKeysString) {
                ...
                for (Map.Entry<?,?> me: jo.entrySet()) {
                    Object v = converter.write(new YormlContextForWrite(me.getValue(), context.getJsonPath()+"/"+me.getKey(), genericValueSubType));
                    out.put(me.getKey(), v);
                }
                isEmpty = out.isEmpty();
                result = out;
                ...
                for (Map.Entry<?,?> me: jo.entrySet()) {
                    Object v = converter.write(new YormlContextForWrite(me.getValue(), context.getJsonPath()+"["+i+"]/value", genericValueSubType));
                    
                    if (me.getKey() instanceof String) {
                        out.add(MutableMap.of(me.getKey(), v));
                    } else {
                        Object k = converter.write(new YormlContextForWrite(me.getKey(), context.getJsonPath()+"["+i+"]/key", genericValueSubType));
