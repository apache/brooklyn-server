 ...
 * under the License.
 */
package io.cloudsoft.amp.containerservice;
...
/**
 * A copy of {@link Repeater} that uses an {@link ExecutorService}
 * to run the checks, allowing them to time out or get cancelled.
 *
 * TODO copy this code back to brooklyn-utils-common
 */
public class ThreadedRepeater implements Callable<Boolean> {

    private static final Logger log = LoggerFactory.getLogger(ThreadedRepeater.class);
     ...
     */
    public ThreadedRepeater(String description) {
        this.description = description != null ? description : "Repeater";
    }

    public static ThreadedRepeater create() {
        return create(null);
    }
    public static ThreadedRepeater create(String description) {
        return new ThreadedRepeater(description);
    }
     ...
     */
    public ThreadedRepeater repeat() {
     ...
     * @return {@literal this} to aid coding in a fluent style.
     */
    public ThreadedRepeater repeat(Runnable body) {
     ...
     * @return {@literal this} to aid coding in a fluent style.
     */
    public ThreadedRepeater repeat(Callable<?> body) {
        checkNotNull(body, "body must not be null");
        this.body = body;
        return this;
    }
     ...
     * @return {@literal this} to aid coding in a fluent style.
     */
    public ThreadedRepeater every(long period, TimeUnit unit) {
     ...
     */
    public ThreadedRepeater every(Duration duration) {
    ...
    public ThreadedRepeater every(groovy.time.Duration duration) {
    ...
    public ThreadedRepeater delayOnIteration(Function<? super Integer,Duration> delayFunction) {
    ...
    public ThreadedRepeater backoff(final Duration initialDelay, final double multiplier, @Nullable final Duration finalDelay) {
    ...
    public ThreadedRepeater backoffTo(final Duration finalDelay) {
     ...
     * @return {@literal this} to aid coding in a fluent style.
     */
    public ThreadedRepeater until(Callable<Boolean> exitCondition) {
        Preconditions.checkNotNull(exitCondition, "exitCondition must not be null");
        this.exitCondition = exitCondition;
        return this;
    }

    public <T> ThreadedRepeater until(final T target, final Predicate<T> exitCondition) {
        Preconditions.checkNotNull(exitCondition, "exitCondition must not be null");
     ...
     * @return {@literal this} to aid coding in a fluent style.
     */
    public ThreadedRepeater rethrowException() {
     ...
     * @return {@literal this} to aid coding in a fluent style.
     */
    public ThreadedRepeater rethrowExceptionImmediately() {
        this.rethrowImmediatelyCondition = Predicates.alwaysTrue();
        return this;
    }

    public ThreadedRepeater rethrowExceptionImmediately(Predicate<? super Throwable> val) {
        this.rethrowImmediatelyCondition = checkNotNull(val, "rethrowExceptionImmediately predicate");
        return this;
    }

    public ThreadedRepeater suppressWarnings() {
     ...
     * @return {@literal this} to aid coding in a fluent style.
     */
    public ThreadedRepeater limitIterationsTo(int iterationLimit) {
     ...
     * @return {@literal this} to aid coding in a fluent style.
     */
    public ThreadedRepeater limitTimeTo(long deadline, TimeUnit unit) {
     ...
     */
    public ThreadedRepeater limitTimeTo(Duration duration) {
        Preconditions.checkNotNull(duration, "duration must not be null");
